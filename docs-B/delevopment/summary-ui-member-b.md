...
- Reader 工具栏提供可勾选的“摘要”按钮和 `Ctrl+Shift+S` 快捷键；隐藏后整个底部 Dock（包括标题栏）消失，再次点击恢复，后台任务和会话内摘要不受影响。
- Summary 使用自定义标题栏，移除会被 Windows/Qt 原生样式重绘成低对比黑色的浮动方块和关闭“×”，改为右侧高对比文字按钮“隐藏”；仍可通过 Reader 工具栏、“视图 → 摘要面板”或 `Ctrl+Shift+S` 恢复。
